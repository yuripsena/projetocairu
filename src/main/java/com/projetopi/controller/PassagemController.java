...
public class PassagemController {
    ...
    public String showNewFaturamento(Model model) {
        return "faturamento";
    }
    ...
    public ModelAndView listarFaturamento(@RequestParam("dataInicio")String dataInicio, @RequestParam("dataFim")String dataFim){
        try {
            ...
            List<Passagem> periodo = new ArrayList<Passagem>();
            for (Passagem lista :passagens) {
                if ((lista.getViagem().getDataSaida().equals(inicio)|| lista.getViagem().getDataSaida().equals(fim)) || (lista.getViagem().getDataSaida().after(inicio) && lista.getViagem().getDataSaida().before(fim))) {
                    periodo.add(lista);
                }
            }
            Double sum = 0.0;
            ModelAndView andView = new ModelAndView("faturamento.html");
            for(Passagem total : periodo) {
                sum = sum+total.getViagem().getValor();
            }
            andView.addObject("sum",sum);
            andView.addObject("psg",periodo);
            return andView;
        }catch (Exception e) {
            e.printStackTrace();
        }
